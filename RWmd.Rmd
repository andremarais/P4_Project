...
No clear link between quality and density.
</br>
```{r, echo = F, message=FALSE}
...
The quality of the wine increases along with higher alcohol content and lower volatile acidity levels. There are quite a few alcohol outliers with wines having a quality rating of 5. _If_ 5 is the average rating for wine, it wouldn't be too naive to assume that a rating of 5 is the go-to rating for wines where the judge can't place it with confidence. This might be the reason why there is a higher variance with the wine with a rating of 5. Also, higher quality wine have smaller volatile acidity levels. Another thing I found interesting is that the variance of the alcohol content increases with the quality of the wine, while the variance of the volatile acidity decreases with better quality wine. This might be due to the wine makers being slightly indifferent about the alcohol content with their wines, and being more concerned about _making a good wine._

Using a random forest model, you can see what the important variables are to help predict quality. Using 500 trees, with replacement, the top 6 features are as follows:
```{r, echo = F, message=FALSE, warning=F, cache=T}
rf.Imp <-randomForest(as.factor(quality) ~ . , data = rw[ ,2:13])

gsub('\\.', ' ',row.names(rf.Imp$importance))[order(rf.Imp$importance, decreasing = T)][1:6]
```
</br>
...
The two outliers in the second last plot shrinks the results too much, taking then out...
</br>
```{r, echo = F, message=FALSE, warning=F}
# indenting with magrittr is still tricky
rw %>%
subset(rw %$%
         total.sulfur.dioxide < rw %$% 
...
Blue dots refer to wines with an alcohol level beneath the mode, while red dots are for wines with an alcohol level above the mode. 
</br>
```{r, echo = F, message=FALSE, warning=F}
# indenting with magrittr is still tricky
rw %>%
...
Comparing the density and total sulfar dioxide between the wine above and below the mode, you can see the variance of the density is much higher with wines with an alcohol above the mode. The other derived variables unfortunately add no new information.
